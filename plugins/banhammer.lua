...
local function pre_process(msg)
  ...
  if msg.action and msg.action.type then
    ...
    if action == 'chat_add_user_link' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] صار بحي حظرتة هذا و دفرتة ! ")-- Save to logs
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "دي" then
         if member_id == from_id then
            send_large_msg(receiver, "حجي دا تحس ما تكدر تطرد نفسك😪🚬")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "ها حجي سلامات ما تكدر تطرد ادمن/مدير/مشرف و اذا ادمن ف اطيح حظك😒😤")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'حظر' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "بحي سلامات ما تكدر تحظر ادمن/مدير/مشرف و اذا المطور ف اطيح حظك😒")
			return
        end
        send_large_msg(receiver, 'العظو @'..member..' ['..member_id..'] تم حظرهة ودعبلتة')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, 'العظو @'..member..' ['..member_id..'] رفعت الحظر عنه')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, 'صار بحي المطور العظو @'..member..' ['..member_id..'] تم حظرهة من كل الكروبات بحي🐸🚬')
		banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, 'العظو @'..member..' ['..member_id..'] رفعت الحظر العام منة')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == "قائمة المحظورين" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "ها حجي سلامات ما تكدر تحظر ادمن/مدير/مشرف واذا مطور ف اطيح حظك😒🚬"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "دا تحس كبدي ما تكدر تحظر نفسك😁✋ !"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
		...
		send_large_msg(receiver, 'العظو ['..matches[2]..'] تم حظره')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر',
...
if matches[1]:lower() == 'دي' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "ها حجي سلامات ما تكدر تطرد ادمن/مدير/مشرف و اذا مطور ف اطيح حظك"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "ها بحي دا تحس شلون تريد تطرد نفسك😂✋ !"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'صار بحي المطور العظو ['..user_id..' ] تم حظره من كل الكروبات بحي🐸🚬'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر عام',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'العظو ['..user_id..' ] تم الغاء الحظر العام منه'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء العام',
  ...
  if matches[1]:lower() == "قائمة الحظر العام" then -- Global ban list
    return banall_list()
  end
end
